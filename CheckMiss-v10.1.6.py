...
DISEASE_SIGNAL_PATTERNS = {
    ...
    "肺结核": re.compile(r"肺结核|结核性胸膜炎|胸膜结核|利福平耐药|病原学阳性|病原学阴性|无病原学结果|涂阳|涂阴|菌阳|菌阴|培阳|仅培阳|未痰检", re.I),
    "肺结核-利福平耐药": re.compile(r"利福平耐药", re.I),
    "肺结核-病原学阳性": re.compile(r"病原学阳性|涂阳|菌阳|培阳|仅培阳", re.I),
    "肺结核-病原学阴性": re.compile(r"病原学阴性|涂阴|菌阴", re.I),
    "肺结核-无病原学结果": re.compile(r"无病原学结果|未痰检", re.I),
    "肺结核-结核性胸膜炎": re.compile(r"结核性胸膜炎|胸膜结核", re.I),
    ...
    "生殖道沙眼衣原体": re.compile(r"生殖道沙眼衣原体感染|生殖道沙眼衣原体|生殖道衣原体|沙眼衣原体生殖道", re.I)
}
...
# 局部排除规则用于步骤2门诊疾病信号生成。
# 原有“排除字段”仍完整保留，用于维护和回溯；这里额外处理被清洗符号拆开的排除语义。
# 例如“梅毒（个人史）”清洗后会变成“梅毒,个人史”，仍应排除本次梅毒信号。
LOCAL_EXCLUDE_RULES = [
    {
        "signal": "梅毒",
        "disease_pattern": re.compile(r"梅毒", re.I),
        "exclude_nearby_pattern": re.compile(r"个人史|既往史|病史|感染史|病程史|史", re.I),
        "window": 1,
    },
    {
        "signal": "乙型肝炎",
        "disease_pattern": re.compile(r"乙肝|乙型肝炎|乙型病毒性肝炎", re.I),
        "exclude_nearby_pattern": re.compile(r"携带者|病毒携带|携带状态", re.I),
        "window": 1,
    },
]
...
def extract_disease_signals(text):
    ...
    signals = {name for name, pattern in DISEASE_SIGNAL_PATTERNS.items() if pattern.search(text)}
    signals.update(extract_tb_detail_signals(text))
    if signals & TB_DETAIL_SIGNALS:
        signals.add(TB_PARENT_SIGNAL)
    return signals
...
def normalized_signal_list(signals):
    """主结果按疾病信号拆分；父级/子级同时存在时，避免生成重复主结果。"""
    display_signals = set(signals)
    if display_signals & HEPATITIS_SPECIFIC_SIGNALS:
        display_signals.discard("病毒性肝炎")
    if display_signals & TB_DETAIL_SIGNALS:
        display_signals.difference_update(TB_DETAIL_SIGNALS)
        display_signals.add(TB_PARENT_SIGNAL)
    return sorted(display_signals)


def split_text_fragments(value):
    """按诊断清洗后的常见分隔符拆分片段，空片段自动丢弃。"""
    if pd.isna(value):
        return []
    return [part.strip() for part in re.split(r"[，,、；;]+", str(value)) if part.strip()]


def build_local_windows(value, max_window=3):
    """把相邻片段重新组合，用于识别被清洗符号拆开的表达，如“痰镜检,+”。"""
    fragments = split_text_fragments(value)
    windows = set()
    for start in range(len(fragments)):
        for size in range(1, max_window + 1):
            part = fragments[start:start + size]
            if part:
                windows.add("".join(part))
                windows.add("，".join(part))
    return "，".join(sorted(windows))


def extract_tb_detail_signals(text):
    """提取肺结核子标签；支持“痰镜检,+”“菌,阴”等被清洗拆开的局部表达。"""
    text = "" if pd.isna(text) else str(text)
    compact_text = re.sub(r"\s+", "", text)
    window_text = build_local_windows(text)
    search_text = f"{compact_text}，{window_text}"

    signals = set()
    positive_pattern = re.compile(
        r"病原学阳性|涂阳|菌阳|培阳|仅培阳|"
        r"(?:涂|涂片|痰涂片|痰检|痰镜检|痰菌|菌|培养|痰培养)[（(]?\+[）)]?",
        re.I
    )
    negative_pattern = re.compile(
        r"病原学阴性|涂阴|菌阴|"
        r"(?:涂|涂片|痰涂片|痰检|痰镜检|痰菌|菌|培养|痰培养)[（(]?[-－阴][）)]?",
        re.I
    )

    if re.search(r"利福平耐药", search_text, re.I):
        signals.add("肺结核-利福平耐药")
    if positive_pattern.search(search_text):
        signals.add("肺结核-病原学阳性")
    if negative_pattern.search(search_text):
        signals.add("肺结核-病原学阴性")
    if re.search(r"无病原学结果|未痰检", search_text, re.I):
        signals.add("肺结核-无病原学结果")
    if re.search(r"结核性胸膜炎|胸膜结核", search_text, re.I):
        signals.add("肺结核-结核性胸膜炎")
    return signals
...
def extract_signals_from_exclude_term(term):
    """从排除字段反推其覆盖的疾病信号；用于阻止匹配字段把已排除信号重新加入。"""
    signals = set(extract_disease_signals(term))
    term = "" if pd.isna(term) else str(term)
    for rule in LOCAL_EXCLUDE_RULES:
        if rule["disease_pattern"].search(term):
            signals.add(rule["signal"])
    return signals


def remove_explicit_exclude_terms(diagnosis_text, exclude_text):
    """从门诊诊断文本中移除步骤1已识别的排除表达，避免步骤2重新生成对应疾病信号。"""
    diagnosis_text = "" if pd.isna(diagnosis_text) else str(diagnosis_text)
    exclude_terms = split_text_fragments(exclude_text)
    removed_signals = set()
    filtered_text = diagnosis_text
    for term in exclude_terms:
        removed_signals.update(extract_signals_from_exclude_term(term))
        filtered_text = re.sub(re.escape(term), "，", filtered_text, flags=re.I)
    return filtered_text, removed_signals


def apply_local_exclude_rules(diagnosis_text):
    """识别被分隔符拆开的局部排除语义，只排除被覆盖的局部疾病命中。"""
    fragments = split_text_fragments(diagnosis_text)
    if not fragments:
        return "", set()
    ...
    for idx, fragment in enumerate(fragments):
        for rule in LOCAL_EXCLUDE_RULES:
            if not rule["disease_pattern"].search(fragment):
                continue
            window = rule.get("window", 1)
            nearby_start = max(0, idx - window)
            nearby_end = min(len(fragments), idx + window + 1)
            nearby_text = "，".join(fragments[nearby_start:nearby_end])
            if rule["exclude_nearby_pattern"].search(nearby_text):
                remove_indexes.add(idx)
                removed_signals.add(rule["signal"])

    kept_fragments = [
        fragment for idx, fragment in enumerate(fragments)
        if idx not in remove_indexes
    ]
    return "，".join(kept_fragments), removed_signals


def extract_effective_clinic_signals(row):
    """生成门诊疾病信号时尊重步骤1排除字段，并补充处理被清洗符号拆开的排除语义。"""
    diagnosis_text = row.get(COLUMN_MAPPING['clinic']['diagnosis'], "")
    match_text = row.get("匹配字段", "")
    exclude_text = row.get("排除字段", "")

    diagnosis_without_explicit, explicit_removed_signals = remove_explicit_exclude_terms(
        diagnosis_text,
        exclude_text
    )
    local_filtered_text, local_removed_signals = apply_local_exclude_rules(
        diagnosis_without_explicit
    )

    diagnosis_signals = extract_disease_signals(local_filtered_text)
    matched_signals = extract_disease_signals(match_text)
    removed_signals = explicit_removed_signals | local_removed_signals

    # 匹配字段作为兜底信号来源，但若该信号只来自已排除表达，则不再重新加入。
    return diagnosis_signals | (matched_signals - removed_signals)


def get_revisit_rule(common_signals):
    """明确标签使用专项规则；不能明确型别时采用30天保守规则。"""
    priority = [
        "乙型肝炎", "丙型肝炎", "艾滋病/HIV", "胎传梅毒", "梅毒", "脊髓灰质炎",
        "麻疹", "狂犬病", "新生儿破伤风",
        "肺结核-利福平耐药", "肺结核-病原学阳性", "肺结核-病原学阴性",
        "肺结核-无病原学结果", "肺结核-结核性胸膜炎", "肺结核",
        "麻风病",
        "流行性腮腺炎", "水痘", "AFP",
        "生殖道沙眼衣原体", "尖锐湿疣", "生殖器疱疹"
    ]
...
def build_judgement_note(row):
    """根据当前行的真实候选状态生成判定说明，避免通用提示造成误解。"""
    if row["优化判定"] == "可疑漏报" and row["身份候选卡数"] > 0:
        return "该门诊疾病信号未找到疾病相关且时间规则可接受的报卡；请结合参考候选和原始数据人工复核"
    if row["优化判定"] == "可疑漏报":
        return "姓名和证件号未匹配到报卡"

    base_note = f"按{row.get('疾病相关程度', '')}、{row.get('适用复诊规则', '')}及时间距离推荐"
    if "负时间差" in str(row.get("最佳候选卡标签", "")):
        return base_note + "；该候选为负时间差，程序按适用复诊规则保留"
    return base_note
...
def evaluate_signal_relation(row):
    ...
    rule_context_signals = set(common_signals)
    if signal == TB_PARENT_SIGNAL and TB_PARENT_SIGNAL in report_signals:
        rule_context_signals.update(report_signals & TB_DETAIL_SIGNALS)
    rule_signal, rule_name, rule_days = get_revisit_rule(rule_context_signals)
    return {
...
def build_candidate_results(data_match, time_unit):
    ...
    signal_base["判定说明"] = signal_base.apply(build_judgement_note, axis=1)
    return signal_base, candidate_rows
...
def protect_excel_text_columns(df, columns):
    """仅在导出副本中保护容易被Excel自动改类型的文本列。"""
    for col in columns:
        if col in df.columns:
            df[col] = df[col].apply(protect_excel_text)


def safe_series(df, column, default=""):
    """返回与DataFrame等长的列；缺列时返回默认值序列，避免空表统计时报错。"""
...
def analysis(data_match, export_txt, export_reported, export_missing, export_duplicate, time_unit,
    ...
    if len(candidate_output) > 0:
        review_mask = (
            (
                (safe_series(candidate_output, "疾病相关分", 0) >= 2)
                | (safe_series(candidate_output, "疾病相关程度", "") == "型别冲突")
            )
            & (
                (safe_series(candidate_output, "是否最佳候选", 0) != 1)
                | safe_series(candidate_output, "未选为最佳卡的原因", "")
                    .fillna("").astype(str).str.strip().ne("")
            )
        )
        candidate_output = candidate_output[review_mask].copy()
    protect_excel_text_columns(
        reported_output,
        ["疾病信号序号", "最佳候选卡ID", "参考候选卡ID"]
    )
    protect_excel_text_columns(
        not_reported_output,
        ["疾病信号序号", "最佳候选卡ID", "参考候选卡ID"]
    ...
    protect_excel_text_columns(
        candidate_output,
        ["疾病信号序号", COLUMN_MAPPING['report']['card_id']]
    )
    ...
    valid_candidates = candidate_output[
        safe_series(candidate_output, dup_col, pd.NA).notna()
    ].copy()
    duplicates = valid_candidates[
        valid_candidates.duplicated(subset=dup_col, keep=False)
    ].sort_values(dup_col) if dup_col in valid_candidates.columns else valid_candidates.copy()
    duplicate_count = len(duplicates)
    negative_delta = int((safe_series(valid_candidates, "负时间差", 0) == 1).sum())
    ...
    suffix_name_candidates = int((safe_series(candidate_details, "姓名匹配方式", "") == "中文姓名后缀兼容").sum())
    exact_name_candidates = int((safe_series(candidate_details, "姓名匹配方式", "") == "完全匹配").sum())
    tag_hint_count = 0
