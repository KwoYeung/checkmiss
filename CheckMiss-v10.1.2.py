...
def match_two_database(data_path1, data_path2, data1=None, data2=None):
    ...
    # 程序内部关联字段：仅用于关联本次输出，不属于医院院内业务编号
...
# ===================== 表格创建函数 =====================
def create_table(rows, align='c'):
    """创建表格并正确设置对齐方式，保留颜色且不破坏排版"""
    from prettytable import PrettyTable
    table = PrettyTable()
    table.border = True
    table.header = False
    
    # 添加所有行数据（保留颜色，仅清理行首多余空格）
    for row in rows:
        # 先保留颜色，仅去除行首普通空格（避免截断颜色代码）
        row_clean = row.lstrip(' ')  # 只删空格，不删颜色代码
        table.add_row([row_clean])
    
    # 在添加数据后设置整个表格的对齐方式
    if align in ['l', 'c', 'r']:
        table.align = align  # 正确设置整个表格的对齐方式
    else:
        table.align = 'c'  # 默认居中
...
def main():
    ...
    while True:
        try:
            # 主菜单分组内容。
            group1 = [f"{Color.GREEN}{Color.BOLD}传染病报卡分析系统---{current_version}{Color.RESET}"]

            group2 = [
                f"{Color.GREEN}author: YueXiuCDC-LGY{Color.RESET}",
